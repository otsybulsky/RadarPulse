# Структурний план книги «RadarPulse: інженерія під доказ»

Цей документ фіксує узгоджену структуру частин та розділів книги, які детально відображають життєвий шлях, архітектуру, кризи продуктивності, бенчмарки, рішення зі спостережуваності (observability) та технологічні рішення системи **RadarPulse** (від Milestone 001 до 037).

---

## [Передмова. виконавчий інженерний вердикт (Executive Engineering Verdict)](book/preface_executive_verdict.md)
...
*   **Сюжет:** Книга не просить довіри, а дає карту доказів: що доведено, де межі тверджень (claims), які продукційні прогалини (production gaps) лишаються і як рецензент може атакувати найсильніші твердження в режимі ворожої рецензії (hostile review).
...
*   **Розділи:**
    *   **[Розділ 1: Двигун на верстаку (Концепція «Лабораторного столу»)](book/chapter_01_lab_table.md)**
    ...
    *   **[Розділ 2: Бінарні лабіринти NEXRAD Level II](book/chapter_02_nexrad_binaries.md)**
...
*   **Розділи:**
    *   **[Розділ 4: Монастир Домену](book/chapter_04_domain_monastery.md)**
    ...
    *   **[Розділ 5: Вартові монастиря (Аналіз тестів архітектури)](book/chapter_05_architecture_guards.md)**
    ...
    *   **[Розділ 6: Залізні контракти домену](book/chapter_06_domain_contracts.md)**
...
*   **Розділи:**
    *   **[Розділ 7: Гроза над шардами](book/chapter_07_hot_partitions.md)**
    ...
    *   **[Розділ 8: Велика міграція топології](book/chapter_08_topology_migration.md)**
    ...
    *   **[Розділ 9: Математика анти-серфінгу (Cooldown & Hysteresis)](book/chapter_09_anti_churn.md)**
    ...
    *   **[Розділ 10: Поштові скриньки воркерів (асинхронний транспорт шардів, Async Shard Transport)](book/chapter_10_async_transport.md)**
        *   *Технічне підґрунтя:* Віха `008` (Retained Async Shard Transport, утриманий асинхронний транспорт шардів).
...
## Частина IV. Війна зі збирачем сміття (Garbage Collector) та керування пам'яттю
*   **Мета:** Хроніка боротьби з алокаційними кризами та оптимізації GC-навантаження.
*   **Бенчмарк-фокус:** Аналіз гейтів Milestone 010/017. Порівняння копіювання зліпка (Snapshot-Copy, 9.95 GB алокацій) та копіювання через пул (Pooled-Copy, 102 MB алокацій).
*   **Розділи:**
    *   **[Розділ 11: Аномалія 9.95 ГБ (Криза асинхронної черги)](book/chapter_11_allocation_anomaly.md)**
        ...
        *   *Сюжет:* Як спроба повністю відв'язати зчитування файлів від процесингу за допомогою копіювання зліпків (`ToOwnedSnapshot`) мало не спалила збирач сміття (Garbage Collector), згенерувавши 9,947,507,832 байт сміття на повному кеші з 198 файлів.
    *   **[Розділ 12: Скарбниця буферів (копіювання через пул, Pooled-Copy)](book/chapter_12_pooled_copy.md)**
    ...
    *   **[Розділ 13: Пастка холодного старту та Prewarm](book/chapter_13_cold_start.md)**
...
## Частина V. Магія паралельного рантайму: паралельність і впорядкована фіксація (Concurrency & Ordered Commit)
*   **Мета:** Розібрати логіку паралельних обчислень із детермінованим виведенням результатів.
*   **Бенчмарк-фокус:** Метрики безпечної паралельності: рантайм активних батчів (active-batch runtime), ціна впорядкованої фіксації (ordered commit tax), ціна перерахунку застарілої топології (stale recompute cost) та вплив на алокацію пам'яті.
*   **Розділи:**
    *   **[Розділ 14: Хаос на іподромі (Паралельність воркерів)](book/chapter_14_concurrency_chaos.md)**
    ...
    *   **[Розділ 15: Старшина черги (`OrderedResultCoordinator`)](book/chapter_15_ordered_coordinator.md)**
    ...
    *   **[Розділ 16: Криза спільного стану](book/chapter_16_mutable_core.md)**
        *   *Технічне підґрунтя:* Віха `021` про кризу спільного стану.
        *   *Сюжет:* Криза спільного стану: архітектурна помилка, коли паралельні воркери перезаписували спільний змінний стан ядра. Розділення рантайму на дві ізольовані фази: розрахунок немутабельних `RadarProcessingBatchDelta` та строго послідовна впорядкована фіксація (Ordered Commit). Аналіз матриці продуктивності (active=4 vs active=1).
    *   **[Розділ 17: Свіжі карти на ходу (перерахунок застарілої топології, Stale Topology Recompute)](book/chapter_17_stale_recompute.md)**
...
*   **Розділи:**
    *   **[Розділ 18: Дипломатичний конверт (`DurableEnvelope`)](book/chapter_18_durable_envelope.md)**
    ...
    *   **[Розділ 19: Слідство у файловій системі (`FileDurableEnvelopeStore`)](book/chapter_19_file_store.md)**
    ...
    *   **[Розділ 20: Принцип зупинки без прихованої неправди (Fail-Closed)](book/chapter_20_fail_closed.md)**
...
*   **Розділи:**
    *   **[Розділ 21: Спеціальні аналітики (користувацькі обробники, Custom Handlers)](book/chapter_21_custom_handlers.md)**
    ...
    *   **[Розділ 22: Контракт злиття дельт (`IMergeableHandler`)](book/chapter_22_delta_merge.md)**
...
*   **Розділи:**
    *   **[Розділ 23: Щит для фронтенду (бекенд для фронтенду, Backend-for-Frontend)](book/chapter_23_bff_shield.md)**
    ...
    *   **[Розділ 24: Кабінет оператора (Angular SPA)](book/chapter_24_operator_ui.md)**
    ...
    *   **[Розділ 25: Демо-пакет під ключ](book/chapter_25_demo_scripts.md)**
...
## Частина IX. Чорний ящик: діагностика, логування та спостережуваність (Diagnostics, Logging, Observability)
*   **Мета:** Пояснити, як система говорить про себе під час нормальної роботи, деградації та блокування.
*   **Бенчмарк-фокус:** Не пропускна здатність (throughput), а доказовість: перша причина блокування (first blocking reason), готовність (readiness), телеметрія черги (queue telemetry), утриманий тиск пам'яті (retained pressure) і доказ місткості (capacity evidence) без перебільшення продукційного логування (production-logging overclaim).
*   **Розділи:**
    *   **[Розділ 26: Чорний ящик RadarPulse](book/chapter_26_observability_logging.md)**
...
*   **[Додаток В: план підготовки до продукційної експлуатації (Production Hardening Plan)](book/appendix_c_production_hardening.md)**
...
*   **[Додаток Г: набір атак рецензента (Reviewer Attack Pack)](book/appendix_d_reviewer_attack_pack.md)**
    *   *Технічне підґрунтя:* [appendix_d_reviewer_attack_pack.md](book/appendix_d_reviewer_attack_pack.md), [appendix_b_claim_evidence_matrix.md](book/appendix_b_claim_evidence_matrix.md), посилання на джерела, тести та віхи (source/test/milestone links).
    *   *Сюжет:* 30-хвилинний маршрут рецензента (30-minute reviewer route) перед довгими додатками та набір найсильніших питань, якими рецензент рівня principal (principal-level reviewer) може атакувати книгу, з маршрутами до відповідей у коді, тестах та доказах із віх (milestone evidence).
*   **[Додаток Д: симульована стенограма ворожої рецензії (Simulated Hostile Reviewer Transcript)](book/appendix_e_simulated_hostile_reviewer_transcript.md)**
...
*   **[Додаток Е: Як відтворити лабораторний стенд на Windows](book/appendix_f_lab_stand_bootstrap.md)**
