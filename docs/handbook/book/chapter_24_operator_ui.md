...
У кабінеті досвідченого детектива завжди є дошка оголошень. На ній розвішані фотографії підозрюваних, карти місцевості, нитки, що зв'язують докази, та велика червона кнопка тривоги. Слідчий не повинен щоразу спускатися в підвал архіваріуса або самостійно розкладати хімічні реактиви, щоб дізнатися, чи готова експертиза. Йому потрібне єдине інформаційне табло, яке відображає поточний статус розслідування у реальному часі. Якщо в якійсь лінії виникає збій, табло має засвітитися червоним кольором і вказати точне місце зламу.
...
Давайте зазирнемо у серце нашого контролера `App` у файлі `app.ts`:
...
Використання обчислюваного сигналу `isBusy` (Computed Signal) — це наш перший лінійний запобіжник на фронтенді. Він об'єднує стани завантаження всіх асинхронних операцій. Якщо хоча б один фоновий процес (наприклад, зчитування історії або запуск симуляції) активний, `isBusy` автоматично приймає значення `true`. Це миттєво відключає (disables) усі кнопки запуску та керування на екрані, запобігаючи повторним клікам оператора та виникненню мережевих черг.
...
Окрім цього, у разі виникнення затримок, оператор може відправляти критичні сигнали керування (Product Controls). Наприклад, якщо пам'ять переповнена, він викликає метод `stopAccepting()`, який блокує прийом нових конвертів, або `drainAccepted()`, щоб очистити чергу:
...
Коли сторінка завантажується вперше, метод `applyInitialUrlState()` зчитує параметри `runId` та `tab` безпосередньо з адресного рядка та автоматично відновлює кабінет детектива у тому ж вигляді, в якому він був залишений.

---

## 24.4. Retained Pressure та First Blocking Reason
...
* **Same-Origin Delivery**: Збірка Angular SPA інтегрується прямо в ресурси `RadarPulse.Http` та віддається з тієї ж адреси, усуваючи потребу в CORS конфігураціях.
* **URL-Restorable State**: Всі важливі параметри фільтрації та обраного запуску мають відновлюватися при перезавантаженні сторінки з URL.
