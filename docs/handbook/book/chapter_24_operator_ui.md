...
Тобто реалізована функціональність — це read-model cockpit: оператор бачить, що запускалося, в якому стані перебуває конвеєр, де перша причина блокування, які batches і sources були оброблені, що повернули handler-и, і які runtime-команди можна безпечно застосувати. Live-карта радара, streaming через WebSocket/SSE і Canvas/WebGL-візуалізація високочастотного потоку ще не входять у цей UI; вони свідомо залишені за межею поточної версії.
...
## 24.5. Поточна межа UI: операторський read-model, а не live-radar canvas
