...
| :--- | :--- | :--- |
| **Діагностика (diagnostics)** | Пояснити стан одного run-а: ready/blocked, first blocking reason, warnings, режим обробника (handler posture) | Реалізовано через read models і product API |
...
## 26.3. Перша причина блокування: одна причина замість хору симптомів
...
## 26.4. Обмежена діагностика: пам'ять важливіша за балакучість
...
## 26.5. Спершу модель, потім OpenTelemetry

OpenTelemetry, structured logging, експортери метрик (metrics exporters) і збирачі трас (trace collectors) — правильні інструменти для production. Але вони мають підсилювати вже названі факти, а не підміняти їх. Якщо підключити інструмент раніше за модель подій, система отримає красиву обгортку навколо неготового змісту.

Для RadarPulse було три можливі шляхи.

**Шлях 1: логувати все відразу.**

Перевага очевидна: у демо багато тексту, рецензент бачить “активність”. Недолік серйозніший: гарячий шлях починає платити за спостереження, а більшість рядків не відповідають на головне питання: що перше зламалося?

**Шлях 2: поставити OpenTelemetry перед стабілізацією семантики рантайму.**

Це дало б знайомі слова: спани, метрики й експортери (spans, metrics, exporters). Але трасування без чіткого `runId`, provider sequence, topology version, envelope state і first blocking reason не пояснює систему. Воно лише малює траєкторію об'єктів, сенс яких ще не зафіксовано.

**Шлях 3: спочатку зробити діагностичний контракт (diagnostic contract).**

Цей шлях менш ефектний, зате сильніший. Спочатку система вчиться називати свої інваріанти: скільки прийнято, скільки оброблено, чи всі зафіксовані (committed), чи є retained pressure, який envelope блокує readiness, який режим обробника використано, яка перша причина блокування.

Лише після цього structured logging стає простим адаптерним шаром, а не спробою вигадати сенс уже в приймачі логів.

RadarPulse обрав третій шлях.

Ціна вибору: у поточній версії не заявлено готову продукційну спостережуваність.

Виграш: коли цей шар з'явиться, він матиме не просто місце для запису, а готову мову фактів.

---

## 26.6. Яким має бути контракт продукційного логування

Якщо завтра RadarPulse треба переносити з lab-table стенда в production, logging/observability має бути першим hardening-кроком перед broker/database/live ingestion (брокер/база даних/live-приймання). Але цей крок має бути точним: не “додати логи”, а винести вже наявні діагностичні факти в стабільний продукційний формат.
...
На цьому місці сильний рецензент має право поставити просте питання:

> “Де ваші логи?”

Слабка відповідь була б: “Ми можемо легко додати `ILogger`”. Це нічого не доводить і не пояснює, які саме факти потраплять у лог.

Сильна відповідь інша:

```text
У поточній версії ми не заявляємо продукційне логування.
Ми вже маємо діагностичний контракт: readiness, blocker, queue telemetry,
durable state, retained pressure, capacity evidence і явно названі межі відповідальності.
Наступний production-hardening крок — адаптер structured logging / metrics / tracing,
який експортує ці типізовані факти без зміни гарячого доменного шляху.
```

Це відповідь, яка завершує логіку всієї книги. Інструмент можна підключити за день. Правильну модель подій, межі шуму, словник кореляції та дисципліну першої причини блокування треба спроектувати.

У RadarPulse ця модель уже почала існувати. Її ще треба винести в продукційний стек спостережуваності, але книга не залишає це питання за кадром. Вона завершується не обіцянкою “ми все залогуємо”, а сильнішою позицією: система вже знає, які факти має сказати про себе.
...
Можна було додати `ILogger` у кожен сервіс і отримати знайомий вигляд production-системи. Але це дало б хибний сигнал: система начебто спостережувана, хоча перша причина блокування, retained pressure, режим обробника і durable state лишалися б розкиданими по рядках. Ми обрали спочатку типізовану діагностику (typed diagnostics first). Ціна вибору — немає заяви “production logging ready” (продукційне логування готове); виграш — кожен майбутній log/metric/trace має джерело правди.
