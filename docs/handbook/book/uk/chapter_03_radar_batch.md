...
## 3.1. Битва за купу: тінь збирача сміття
...
## 3.2. Рішення: сержант [`RadarStreamEvent`](../../../../src/Domain/Streaming/Streams/Models/RadarStreamEvent.cs) як некерована структура
...
## 3.3. Потоковий шар: [`RadarEventBatch`](../../../../src/Domain/Streaming/Batches/Models/RadarEventBatch.cs)
...
Отже, батч уже має компактну форму: події лежать щільно, payload живе в одному спільному сховищі, а домен бачить не купу дрібних об'єктів, а контрольований пакет. Але між сирими байтами NEXRAD і цим батчем лишається ще один небезпечний перехід — момент, коли зовнішній бінарний формат стає нашою внутрішньою структурою.
...
* Практичний протокол профілювання: [Додаток 1](appendix_a_profiling.md)
