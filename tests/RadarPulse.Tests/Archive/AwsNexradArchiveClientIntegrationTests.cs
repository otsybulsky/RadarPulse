...
{
    private const long MaxIntegrationDownloadBytes = 25_000_000;
    ...
    [IntegrationFact]
    public async Task DownloadFileAsyncDownloadsSmallPublicAwsObject()
    {
        using var httpClient = new HttpClient();
        var client = new AwsNexradArchiveClient(httpClient);
        var request = new HistoricalArchiveRequest(
        ...
        var manifest = await client.BuildManifestAsync(request, CancellationToken.None);
        var file = Assert.Single(manifest.Files);
        Assert.InRange(file.SizeBytes, 1, MaxIntegrationDownloadBytes);

        await using var destination = new MemoryStream();
        await client.DownloadFileAsync(file, destination, CancellationToken.None);
